...
void max30102_FIFO_ReadWords(u8 Register_Address, u16 Word_Data[][2],
                             u8 count) {
  u8 i = 0;
  u8 no = count;
  u8 data1, data2;
...
void MAX30102_Init(void) {
  ...
  max30102_Bus_Write(REG_SPO2_CONFIG,
                     0x27); // SPO2_ADC range = 4096nA, SPO2 sample rate (100
                            // Hz), LED pulseWidth (400uS)
  max30102_Bus_Write(REG_LED1_PA, 0x24); // Choose value for ~ 7mA for LED1
  max30102_Bus_Write(REG_LED2_PA, 0x24); // Choose value for ~ 7mA for LED2
  max30102_Bus_Write(REG_PILOT_PA,
...
void MAX30102_IIC_ReadBytes(u8 deviceAddr, u8 writeAddr, u8 *data,
                            u8 dataLength) {
  u8 i;
  MAX30102_IIC_Start();
...
const uint16_t auw_hamm[31] = {41, 276, 512, 276,
                               41}; // Hamm=  long16(512* hamming(5)');
 ...
 */
{
  ...
  maxim_find_peaks(an_dx_peak_locs, &n_npks, an_dx, BUFFER_SIZE - HAMMING_SIZE,
  ...
  // calibration ratio finding AC/DC maximum of raw ir * red between two valley
  // locations
  n_ratio_average = 0;
  n_i_ratio_count = 0;
  ...
  for (k = 0; k < n_exact_ir_valley_locs_count - 1; k++) {
    ...
    if (an_exact_ir_valley_locs[k + 1] - an_exact_ir_valley_locs[k] > 10) {
      ...
      n_y_ac = an_y[an_exact_ir_valley_locs[k]] +
               n_y_ac / (an_exact_ir_valley_locs[k + 1] -
                         an_exact_ir_valley_locs[k]);

      n_y_ac = an_y[n_y_dc_max_idx] -
               n_y_ac; // subracting linear DC compoenents from raw
      ...
      n_x_ac = an_x[an_exact_ir_valley_locs[k]] +
               n_x_ac / (an_exact_ir_valley_locs[k + 1] -
                         an_exact_ir_valley_locs[k]);
      n_x_ac = an_x[n_y_dc_max_idx] -
               n_x_ac; // subracting linear DC compoenents from raw
      n_nume =
          (n_y_ac * n_x_dc_max) >> 7; // prepare X100 to preserve floating value
      n_denom = (n_x_ac * n_y_dc_max) >> 7;
      if (n_denom > 0 && n_i_ratio_count < 5 && n_nume != 0) {
        an_ratio[n_i_ratio_count] =
            (n_nume * 20) /
            n_denom; // formular is ( n_y_ac *n_x_dc_max) / ( n_x_ac
                     // *n_y_dc_max) ;
                     // ///*************************n_nume原来是*100************************//
        n_i_ratio_count++;
      }
  ...
  if (n_middle_idx > 1)
    n_ratio_average =
        (an_ratio[n_middle_idx - 1] + an_ratio[n_middle_idx]) / 2; // use median
  else
  ...
  if (n_ratio_average > 2 && n_ratio_average < 184) {
    ...
    *pch_spo2_valid = 1; //  float_SPO2 =  -45.060*n_ratio_average*
                         //  n_ratio_average/10000 + 30.354 *n_ratio_average/100
                         //  + 94.845 ;  // for comparison with table
  } else {
    *pn_spo2 = -999; // do not use SPO2 since signal ratio is out of range
    *pch_spo2_valid = 0;
  }
