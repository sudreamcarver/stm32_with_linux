 ...
 *
 * This is the code documentation for the MAXREFDES117# subsystem reference
 * design.
 *
 ...
 *
 * \image html MAXREFDES117_Block_Diagram.png "MAXREFDES117# System Block
 * Diagram"
 *
 * \image html MAXREFDES117_firmware_Flowchart.png "MAXREFDES117# Firmware
 * Flowchart"
 *
 */
#include "MAX30102.h"
...
DigitalIn
    INT(PB_7); // pin PB7 connects to the interrupt output pin of the MAX30102
DigitalOut led(PC_13); // PC13 connects to the on board user LED

// the setup routine runs once when you press reset:
int main() {
  uint32_t un_min, un_max,
      un_prev_data; // variables to calculate the on-board LED brightness that
                    // reflects the heartbeats
  int i;
  ...
  n_ir_buffer_length =
      500; // buffer length of 100 stores 5 seconds of samples running at 100sps
  ...
  while (1) {
    ...
    for (i = 400; i < 500; i++) {
      ...
      if (aun_red_buffer[i] >
          un_prev_data) // just to determine the brightness of LED according to
                        // the deviation of adjacent two AD data
      {
